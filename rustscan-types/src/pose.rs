...
impl SE3 {
    ...
    /// Get the underlying glam quaternion
    pub fn quat(&self) -> Quat {
        self.rotation
    }

    /// Get the underlying glam translation vector
...
mod tests {
    ...
    #[test]
    fn test_serde() {
        let pose = SE3::from_axis_angle(&[0.1, 0.2, 0.3], &[1.0, 2.0, 3.0]);
        let json = serde_json::to_string(&pose).unwrap();
        let decoded: SE3 = serde_json::from_str(&json).unwrap();
