use burn::tensor::{DType, Int, Shape, Tensor, TensorMetadata};
...
pub async fn prefix_sum_u32<B: PrefixSumBackend>(
    input: Tensor<B, 1, Int>,
    _device: &B::Device,
) -> Result<Tensor<B, 1, Int>, String> {
    let primitive = B::prefix_sum_u32_primitive(input.into_primitive())?;
    Ok(Tensor::from_primitive(primitive))
}

#[cfg(test)]
mod tests {
    use super::prefix_sum_u32;
    ...
    async fn assert_prefix(input: &[u32]) {
        ...
        let result = prefix_sum_u32::<GsBackendBase>(tensor, &device)
            .await
            .expect("prefix sum");
