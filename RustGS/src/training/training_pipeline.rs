...
/// Error type for scene IO operations
#[derive(Debug)]
...
impl std::fmt::Display for SceneIoError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SceneIoError::Io(e) => write!(f, "IO error: {}", e),
            SceneIoError::InvalidFormat(s) => write!(f, "Invalid format: {}", s),
        }
    }
}
...
/// Save scene to PLY format (placeholder - full implementation in Story 9-7)
pub fn save_scene_ply(
    _path: &Path,
    _gaussians: &[Gaussian],
    _metadata: &SceneMetadata,
) -> Result<PathBuf, SceneIoError> {
    // Placeholder - will be implemented in Story 9-7 when scene_io.rs is migrated
    Err(SceneIoError::Io(std::io::Error::new(
        std::io::ErrorKind::Unsupported,
        "Scene PLY export will be implemented in Story 9-7",
    )))
}

/// Load scene from PLY format (placeholder - full implementation in Story 9-7)
pub fn load_scene_ply(_path: &Path) -> Result<(Vec<Gaussian>, SceneMetadata), SceneIoError> {
    // Placeholder - will be implemented in Story 9-7 when scene_io.rs is migrated
    Err(SceneIoError::Io(std::io::Error::new(
        std::io::ErrorKind::Unsupported,
        "Scene PLY import will be implemented in Story 9-7",
    )))
}
...
impl TrainingState {
    pub fn new(width: usize, height: usize) -> Self {
        let (fx, fy, cx, cy) = default_camera_intrinsics();
        Self {
            iteration: 0,
            gaussians: Vec::new(),
...
mod tests {
    ...
    fn test_training_state_creation() {
        let state = TrainingState::new(1920, 1440);
        assert_eq!(state.iteration, 0);
        assert!(state.gaussians.is_empty());
        assert_eq!(state.renderer.width, 1920);
        assert_eq!(state.renderer.height, 1440);
    }

    #[test]
    fn test_training_state_with_intrinsics() {
        let state = TrainingState::with_intrinsics(640, 480, 500.0, 500.0, 320.0, 240.0);
        assert_eq!(state.fx, 500.0);
        assert_eq!(state.fy, 500.0);
