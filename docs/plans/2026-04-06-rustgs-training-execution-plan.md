...
**Architecture:** 生产训练继续以 `rustgs::train()` -> `training::train()` -> `train_stream::train()` -> `metal_trainer::train_loaded()` 为主干，`topology.rs` 负责 densify / prune / opacity reset 调度，`eval.rs` 和 `examples/evaluate_psnr.rs` 负责质量汇总。`training_pipeline.rs` 继续只保留 legacy/reference 责任；Brush 只做 selective migration，不新增并行训练主线。
...
1. 当前生产训练入口已经不是 `training_pipeline.rs`。
   ...
   - 训练 orchestration 在 `RustGS/src/training/train_stream.rs`
...
3. 当前最真实的问题不是“完全训不起来”，而是：
   ...
   - 生产模块边界虽然已经成型，但仍有局部遗留耦合，例如 `topology.rs` 还在借用 `training_pipeline::TrainingConfig` 默认值
...
### Task 2: 去掉生产路径对 legacy defaults 的隐式依赖

**Files:**
- Modify: `RustGS/src/training/topology.rs`
- Modify: `RustGS/src/training/mod.rs`
- Modify: `RustGS/src/training/train_stream.rs`
- Test: `RustGS/src/training/mod.rs`
- Test: `RustGS/src/training/train_stream.rs`

**Step 1: 把 topology 默认值完全收口到当前生产配置面**

- 移除 `topology.rs` 对 `training_pipeline::TrainingConfig` 默认值的借用
- 只允许 `TrainingConfig` / `LiteGsConfig` 作为生产路径的参数来源

**Step 2: 明确 legacy 和 litegs 的责任边界**

- `LegacyMetal` 仍保留现状行为
- `LiteGsMacV1` 的 topology 语义只通过当前 `mod.rs` 配置面暴露，不再从 legacy helper 偷拿默认值

**Step 3: 跑 route/config regression**

Run: `cargo test --manifest-path RustGS/Cargo.toml --features gpu train_stream::tests -- --nocapture`
...
```bash
git add RustGS/src/training/topology.rs RustGS/src/training/mod.rs RustGS/src/training/train_stream.rs
...
**Files:**
- Modify: `RustGS/src/training/topology.rs`
- Modify: `RustGS/src/training/splat_params.rs`
- Modify: `RustGS/src/training/optimizer_state.rs`
- Modify: `RustGS/src/training/metal_trainer.rs`
...
- 调度留在 `topology.rs`
- 参数重建留在 `splat_params.rs` / `optimizer_state.rs`
...
```bash
git add RustGS/src/training/topology.rs RustGS/src/training/splat_params.rs RustGS/src/training/optimizer_state.rs RustGS/src/training/metal_trainer.rs RustGS/tests/tum_training.rs
...
- `docs/RustGS-Brush-Migration-Architecture-2026-04-05.md`
- `docs/plans/2026-04-06-rustgs-training-execution-plan.md`

## Docs Removed / Superseded

以下文档已不再适合作为当前执行依据：

- `docs/RustGS-LiteGS-Parity-Status-2026-04-04.md`
- `docs/RustGS-Smoke-Benchmark-Report-2026-04-06.md`
- `docs/RustGS-Training-Report.md`
- `.claude/worktrees/rustgs-opt/docs/RustGS-Brush-Training-Refactor-Plan-2026-04-06.md` 仅保留为外部分析输入，不再作为执行计划来源
