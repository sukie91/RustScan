...
- depth loss 的 valid-depth mask 与 backward scale 已开始显式标定，但仍缺 LiteGS side-by-side 曲线与阈值对照
...
最近几轮在 `rustgs-opt` 上新增落地的工作包括：

- `ac71165`：实现 LiteGS sparse gradient optimizer 第一版语义
- `95a735d`：放开 LiteGS profile 下的 depth supervision 开关
- `4229a97`：补充 LiteGS parity regression 覆盖
- `9b4baf6`：同步更新 parity progress 文档状态

这些提交带来的实际结果是：

- `sparse_grad / enable_depth / clustered` 三者已经可以在训练路径中组合
- runtime telemetry 与 parity report 都已经能记录 depth 与 LiteGS 关键配置
- depth supervision 现在会显式记录 valid-depth pixel 数、depth grad scale 和 loss curve sample
- fixture parity report 已支持自动加载 `parity-reference.json` 并计算 depth/total loss curve delta
- CLI parity report 写出路径现在已有端到端回归覆盖，可在临时 workspace 中验证 reference fixture 自动发现与 comparison 生成
- parity report 现在会生成结构化 gate 摘要，区分 `passed / failed / missing_reference / incomplete`
- 当前 codebase 已从“补功能入口”转向“做 parity 校准与性能化”
...
- 将 depth backward scale 从“按总像素数归一化”修正为“按 valid-depth 像素数归一化”
- 将 depth valid mask 统一为 `finite && > 0` 语义
- 增加 depth telemetry / parity report 字段，记录 `depth_valid_pixels`、`depth_grad_scale` 和 sampled loss curve

结果：

- `enable_depth` 已经从“代码存在但不可用”推进到“可正式开启”
- RustGS 现在至少已经对 depth 权重常量、valid-depth mask 和 backward scale 做了显式内部对齐
- 后续重点从“放开开关”转为“做 LiteGS side-by-side depth curve 校准”

限制：

- 当前完成的是 RustGS 内部语义校准与 telemetry 采样，不是和 LiteGS 真值的最终对照
- 还没有完成对 LiteGS depth loss 曲线、容忍阈值和 reference fixture 的显式 side-by-side 校准
...
- `cargo test -p rustgs depth_backward_scale_uses_only_valid_depth_samples`
...
- `cargo test -p rustgs litegs_parity_report_uses_runtime_telemetry_when_available`
- `cargo test -p rustgs litegs_parity_report_populates_reference_comparison_from_workspace_fixture`
- `cargo test -p rustgs parity_harness -- --nocapture`
- `cargo test -p rustgs parity_report_round_trips_through_json`
...
- report 级 gate 已能对 no-NaN / no-OOM / export roundtrip / gaussian-count delta 做结构化判定
- 但当前仍以定向单测和组件级回归为主

需要完成的事情：

- 将真实 LiteGS `parity-reference.json` check in 到 canonical fixture 路径
- 对关键 loss term、gaussian 数量演化和训练曲线做 LiteGS side-by-side 对比
- 明确 sparse / depth / clustered 组合下的容忍阈值，并把当前 `missing_reference` gate 升级成真实回归门禁
...
- valid-depth mask、backward scale 和 telemetry curve sample 已显式接入
- gate schema 已预留 depth / total curve tolerance 字段，但默认还未启用
- 但目前仍缺 LiteGS 对照标定

需要完成的事情：

- 用 reference fixture 对比 RustGS / LiteGS 的 depth term 曲线
- 明确 `depth_valid_pixels`、`depth_grad_scale` 和 LiteGS 对应项之间的容忍阈值
- 补充开启 depth supervision 的 side-by-side 回归
...
如果按当前状态继续推进，建议直接按下面的可执行顺序做：

1. 建一个最小 fixture 级回归
   - 固定输入场景
   - 固定 LiteGS profile 配置
   - 固定输出 parity report 字段检查
2. 对 depth loss 做显式标定
   - 核对 weight
   - 核对 valid-depth mask
   - 核对训练中 telemetry 的 depth term 变化
3. 补 sparse / clustered 的性能观察
   - 记录 visible primitive 数量
   - 记录 sparse update 前后 step 时间
   - 判断是否值得上 compacted tensor / visible chunk 结构
4. 最后再进入 `learnable_viewproj` 性能版
